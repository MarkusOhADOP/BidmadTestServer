...
<script>
  ...
  (async function main() {
    try {
      ...
      var resp = await fetch("/bid/" + adInfo.zoneId, {
        method: "POST",
      ...
      if (window._adType === "VAST") {
        ...
        await new Promise(function(resolve, reject) {
          ...
          renderVAST(adm, adInfo.width, adInfo.height).catch(reject);
        });
      } else {
        renderScript(adm, adInfo.width, adInfo.height);
      }
